...
const SYSTEMS: Record<string, string> = {
  ...
  build: `You are Razen — Build mode. Staff-level engineer and product builder. Produce concrete, implementation-ready output, not vague advice. When asked to build or improve something, think through product UX, system design, edge cases, quality, and execution depth before answering. Prefer strong structure, specific recommendations, and production-minded detail over generic summaries.`,
  write: `You are Razen — Write mode. World-class editor. Match the requested register precisely. Default to sharp, specific, high-signal prose. Return polished final copy, not rambling commentary.`,
  plan: `You are Razen — Plan mode. Elite operator and chief of staff. Turn ambiguity into a decisive plan with assumptions, sequencing, risks, mitigation, and definition of done. Push for depth and clarity.`,
};
...
function route(tier: Tier, mode: Mode, msgChars: number): Routed {
  ...
  if (tier === "elite") {
    if (mode === "build") return { model: "google/gemini-3-flash-preview", cost: heavy ? 12 : 9, reasoning: "high" };
    if (mode === "plan") return { model: "google/gemini-3-flash-preview", cost: heavy ? 10 : 8, reasoning: "high" };
    if (mode === "write") return { model: "google/gemini-3-flash-preview", cost: heavy ? 8 : 6, reasoning: "medium" };
    return { model: "google/gemini-3-flash-preview", cost: heavy ? 5 : 3, reasoning: "medium" };
  }

  if (tier === "pro") {
    if (mode === "build") return { model: "google/gemini-3-flash-preview", cost: heavy ? 8 : 6, reasoning: "medium" };
    if (mode === "plan") return { model: "google/gemini-3-flash-preview", cost: heavy ? 7 : 5, reasoning: "medium" };
    if (mode === "write") return { model: "google/gemini-3-flash-preview", cost: heavy ? 5 : 4, reasoning: "medium" };
    return { model: "google/gemini-3-flash-preview", cost: heavy ? 3 : 2, reasoning: "low" };
  }

  if (mode === "build") return { model: "google/gemini-3-flash-preview", cost: heavy ? 5 : 4, reasoning: "medium" };
  if (mode === "plan") return { model: "google/gemini-3-flash-preview", cost: 4, reasoning: "medium" };
